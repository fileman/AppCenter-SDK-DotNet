...
namespace Microsoft.AppCenter.Crashes
{
    ...
    internal class ImageReader
    {
        ...
        /// </summary>
        public const int SignatureOffsetLocation = 0x3C;
        ...
        public ImageReader(IntPtr imageBase)
        {
        ...
        {
            int peHeaderOffset = ReadDwordAtFileOffset(SignatureOffsetLocation);
            int peOptionalHeaderOffset = peHeaderOffset + 4 + SizeofCOFFFileHeader;
            int sizeOfImage = ReadDwordAtFileOffset(peOptionalHeaderOffset + 56);
            IntPtr endAddress = _imageBase + sizeOfImage;
            ...
            if (magic == PEMagic.PEMagic32)
            {
                optionalHeaderDirectoryEntriesOffset = peOptionalHeaderOffset +
                                                       SizeofOptionalHeaderStandardFields32 +
                                                       SizeofOptionalHeaderNTAdditionalFields32;
            }
            else
            {
                optionalHeaderDirectoryEntriesOffset = peOptionalHeaderOffset +
                                                       SizeofOptionalHeaderStandardFields64 +
                                                       SizeofOptionalHeaderNTAdditionalFields64;
            }

            DirectoryEntry debugDirectoryEntry = ReadDebugDirectoryEntry(optionalHeaderDirectoryEntriesOffset);
            DebugDirectory[] debugDirectories = ReadDebugDirectories(debugDirectoryEntry);
            return GetCodeViewDebugData(debugDirectories, endAddress);
        }
        ...
        {
            Marshal.Copy(_imageBase + offset, fileBytes, 0, fileBytes.Length);
        }
        ...
        private DirectoryEntry ReadDebugDirectoryEntry(int optionalHeaderDirectoryEntriesOffset)
        {
            ...
            {
                RelativeVirtualAddress = ReadDwordAtFileOffset(optionalHeaderDirectoryEntriesOffset + DebugTableDirectoryOffset),
                Size = ReadDwordAtFileOffset(optionalHeaderDirectoryEntriesOffset + DebugTableDirectoryOffset + 4)
            };
            return entry;
        }
        ...
        private DebugDirectory[] ReadDebugDirectories(DirectoryEntry debugDirectoryEntry)
        {
            ...
            BinaryReader reader = ReadAtRelativeVirtualAddress(debugDirectoryEntry.RelativeVirtualAddress, debugDirectoryEntry.Size);
            int countDirectories = (int)(debugDirectoryEntry.Size / SizeofDebugDirectory);
        ...
        private CodeViewDebugData GetCodeViewDebugData(DebugDirectory[] debugDirectories, IntPtr endAddress)
        {
            foreach (DebugDirectory debugDirectory in debugDirectories)
            {
                ...
                BinaryReader reader = ReadAtRelativeVirtualAddress((int)debugDirectory.AddressOfRawData, (int)debugDirectory.SizeOfData);
                int signature = reader.ReadInt32();
                if (signature != ImageReader.CodeViewSignature)
                {
                    return null;
                }
                ...
                CodeViewDebugData codeView = new CodeViewDebugData(
                                        ...
                                        ReadNullTerminatedUTF8String(reader),
                                        endAddress);
                return codeView;
            }
